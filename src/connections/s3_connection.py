...
class s3_operations:
    def __init__(self, bucket_name, aws_access_key, aws_secret_key, region_name="us-east-1"):
        ...
        self.s3_client = boto3.client(
            's3',
            aws_access_key_id=aws_access_key,      # Map to boto3's expected parameter name
            aws_secret_access_key=aws_secret_key,  # Map to boto3's expected parameter name
            region_name=region_name
        )
        logging.info(f"Data Ingestion from S3 bucket '{bucket_name}' initialized")
